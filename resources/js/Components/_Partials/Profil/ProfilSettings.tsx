...
const ProfilSettings: React.FC<ProfilSettingsProps> = ({user}) => {
    ...
    const navs = [{
        name: "My Profile", icon: <TbUser/>,
    }, {
        name: "Email et Réseaux Sociaux", icon: <TbLinkPlus/>,
    }, {
        name: "Photos et Vidéos", icon: <TbBoxMultiple/>,
    }, {
        name: "Security & Password", icon: <TbShieldLock/>,
    },];
    ...
    return (<>
        <div className="mb-4">
            {/* User Profile Card */}
            <div
                className="p-3 md:p-5 bg-white border border-gray-200 shadow-2xs rounded-xl dark:bg-neutral-800 dark:border-neutral-700">

                <ImageCropper type={"profile"} translations={{
                    Text: {
                ...
                {/* Header */}
                <div
                    className="mt-4 md:mt-7 -mb-0.5 flex flex-col md:flex-row md:justify-between md:items-center gap-3">
                    {/* Nav */}
                    <div
        ...
        "autoCentering": true
                    ...
                    >
                        <nav
                            className="hs-scroll-nav-body flex flex-nowrap gap-x-1 overflow-x-auto [&::-webkit-scrollbar]:h-0 snap-x snap-mandatory pb-1.5">
                            {navs.map((nav, index) => (<span
                                key={index}
